...
export default function PaymentSettingsModal({ onClose }: PaymentSettingsModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (gatewayType === "XENDIT" && !apiKey.trim()) {
              ...
              <div className="space-y-4">
                ...
                {gatewayType === "XENDIT" && (
                  <>
                    ...
                    <div className="space-y-1.5">
                      <label className="text-[11px] font-semibold text-[#09090b]">Secret API Key</label>
                      <input type="password" value={apiKey} onChange={e => setApiKey(e.target.value)} placeholder="xnd_development_… or xnd_production_…" className="w-full h-10 border border-[#e5e5e5] px-3 font-mono text-[13px] focus:outline-none focus:border-[#09090b]" />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[11px] font-semibold text-[#09090b]">Callback token (x-callback-token)</label>
                      <input type="password" value={webhookSecret} onChange={e => setWebhookSecret(e.target.value)} placeholder="Xendit dashboard callback token" className="w-full h-10 border border-[#e5e5e5] px-3 font-mono text-[13px] focus:outline-none focus:border-[#09090b]" />
