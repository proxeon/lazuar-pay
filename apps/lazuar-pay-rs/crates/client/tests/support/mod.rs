//! Shared Postgres 16 + axum bind for client/cli HTTP tests.
    ...
    &PG.get_or_init(|| {
        std::thread::spawn(|| {
            let container = PgImage::default()
                .with_tag("16-alpine")
                .start()
                .expect("Pay client tests require Docker/Testcontainers Postgres 16");
...
/// Bind `api::router(testing_state)` on an ephemeral port. Fake One: `lzr_sk_test`.
