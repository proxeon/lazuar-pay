...
use workers::razorpay_remote::RazorpayRemote;
use workers::secret_box::SecretBox;
use workers::stripe_remote::StripeRemote;
use workers::xendit_remote::XenditRemote;
...
async fn mint_session(
    ...
    let minted = apply(
        ...
        ApplyCmd::Mint(MintSpec {
            ...
            quoted: if rail == RailId::RAZORPAY {
                Money::from_quoted_str("10.00", Currency::INR).unwrap()
            } else {
                myr10()
            },
...
async fn vault_chip(pool: &sqlx::PgPool, tenant: &str) {
    let box_ = SecretBox::new(SecretBox::testing_fallback_key());
    let ct = box_.protect_str("chip_sk").unwrap();
    let wh = box_
        .protect_str("-----BEGIN PUBLIC KEY-----\nM\n-----END PUBLIC KEY-----")
        .unwrap();
    storage::upsert_chip(pool, tenant, &ct, &wh, "p_sk", Some("test"), "brand_1")
        .await
        .unwrap();
}

fn chip_fixture(name: &str) -> String {
...
#[tokio::test]
async fn chip_remote_paid_fixture_takes() {
    let _g = PSYNC.lock().await;
    let pool = pool().await;
    let (tenant, payment_id, _a, _s) = mint_session(&pool, RailId::CHIP, true).await;
    vault_chip(&pool, tenant.as_str()).await;
    let fake = rails::chip::FakeChip::default();
    fake.set_retrieve(200, &chip_fixture("psync_paid.json"));
    let remote = ChipRemote::fake(pool.clone(), SecretBox::testing_fallback_key(), fake);
    psync::process_batch(&pool, &remote).await.unwrap();
    let charges: i64 =
        sqlx::query_scalar("SELECT count(*)::bigint FROM pay_rs.charges WHERE payment_id = $1")
            .bind(payment_id.as_uuid())
    ...
    let cap: Option<String> =
        sqlx::query_scalar("SELECT capture_id FROM pay_rs.attempts WHERE payment_id = $1")
            ...
            .await
            .unwrap();
    assert_eq!(cap.as_deref(), Some("purch_1"));
}

#[tokio::test]
async fn chip_remote_pending_stays_session_live() {
    let _g = PSYNC.lock().await;
    let pool = pool().await;
    let (tenant, payment_id, attempt_id, _s) = mint_session(&pool, RailId::CHIP, true).await;
    vault_chip(&pool, tenant.as_str()).await;
    let fake = rails::chip::FakeChip::default();
    fake.set_retrieve(200, &chip_fixture("psync_pending.json"));
    let remote = ChipRemote::fake(pool.clone(), SecretBox::testing_fallback_key(), fake);
    psync::process_batch(&pool, &remote).await.unwrap();
    let ast: String = sqlx::query_scalar("SELECT status FROM pay_rs.attempts WHERE id = $1")
        .bind(attempt_id.as_uuid())
...
async fn vault_xendit(pool: &sqlx::PgPool, tenant: &str) {
    let box_ = SecretBox::new(SecretBox::testing_fallback_key());
    let ct = box_.protect_str("xnd_sk").unwrap();
    let wh = box_.protect_str("tok_1").unwrap();
    storage::upsert_xendit(pool, tenant, &ct, &wh, "d_sk", Some("test"))
        .await
        .unwrap();
}

fn xendit_fixture(name: &str) -> String {
    std::fs::read_to_string(format!(
        "{}/../rails/tests/fixtures/xendit/{name}",
        env!("CARGO_MANIFEST_DIR")
    ))
    .unwrap()
}
...
#[tokio::test]
async fn xendit_remote_paid_fixture_takes() {
    let _g = PSYNC.lock().await;
    let pool = pool().await;
    let (tenant, payment_id, _a, _s) = mint_session(&pool, RailId::XENDIT, true).await;
    vault_xendit(&pool, tenant.as_str()).await;
    let fake = rails::xendit::FakeXendit::default();
    fake.set_retrieve(200, &xendit_fixture("psync_paid.json"));
    let remote = XenditRemote::fake(pool.clone(), SecretBox::testing_fallback_key(), fake);
    psync::process_batch(&pool, &remote).await.unwrap();
    let charges: i64 =
        sqlx::query_scalar("SELECT count(*)::bigint FROM pay_rs.charges WHERE payment_id = $1")
            .bind(payment_id.as_uuid())
...
#[tokio::test]
async fn xendit_remote_pending_stays_session_live() {
    let _g = PSYNC.lock().await;
    let pool = pool().await;
    let (tenant, payment_id, attempt_id, _s) = mint_session(&pool, RailId::XENDIT, true).await;
    vault_xendit(&pool, tenant.as_str()).await;
    let fake = rails::xendit::FakeXendit::default();
    fake.set_retrieve(200, &xendit_fixture("psync_pending.json"));
    let remote = XenditRemote::fake(pool.clone(), SecretBox::testing_fallback_key(), fake);
    psync::process_batch(&pool, &remote).await.unwrap();
    let ast: String = sqlx::query_scalar("SELECT status FROM pay_rs.attempts WHERE id = $1")
        .bind(attempt_id.as_uuid())
...
#[tokio::test]
async fn xendit_remote_expired_stays_session_live() {
    let _g = PSYNC.lock().await;
    let pool = pool().await;
    let (tenant, payment_id, attempt_id, _s) = mint_session(&pool, RailId::XENDIT, true).await;
    vault_xendit(&pool, tenant.as_str()).await;
    let fake = rails::xendit::FakeXendit::default();
    fake.set_retrieve(200, &xendit_fixture("psync_expired.json"));
    let remote = XenditRemote::fake(pool.clone(), SecretBox::testing_fallback_key(), fake);
    psync::process_batch(&pool, &remote).await.unwrap();
    let ast: String = sqlx::query_scalar("SELECT status FROM pay_rs.attempts WHERE id = $1")
        .bind(attempt_id.as_uuid())
...
async fn vault_razorpay(pool: &sqlx::PgPool, tenant: &str) {
    let box_ = SecretBox::new(SecretBox::testing_fallback_key());
    let ct = box_.protect_str("rzp_test:secret").unwrap();
    let wh = box_.protect_str("wh_rzp").unwrap();
    storage::upsert_razorpay(pool, tenant, &ct, &wh, "test", Some("test"))
        .await
        .unwrap();
}

fn razorpay_fixture(name: &str) -> String {
    std::fs::read_to_string(format!(
        "{}/../rails/tests/fixtures/razorpay/{name}",
        env!("CARGO_MANIFEST_DIR")
    ))
    .unwrap()
}
...
#[tokio::test]
async fn razorpay_remote_paid_fixture_takes() {
    let _g = PSYNC.lock().await;
    let pool = pool().await;
    let (tenant, payment_id, _a, _s) = mint_session(&pool, RailId::RAZORPAY, true).await;
    vault_razorpay(&pool, tenant.as_str()).await;
    let fake = rails::razorpay::FakeRazorpay::default();
    fake.set_retrieve(200, &razorpay_fixture("psync_paid.json"));
    let remote = RazorpayRemote::fake(pool.clone(), SecretBox::testing_fallback_key(), fake);
    psync::process_batch(&pool, &remote).await.unwrap();
    let charges: i64 =
        sqlx::query_scalar("SELECT count(*)::bigint FROM pay_rs.charges WHERE payment_id = $1")
            .bind(payment_id.as_uuid())
...
#[tokio::test]
async fn razorpay_remote_created_stays_session_live() {
    let _g = PSYNC.lock().await;
    let pool = pool().await;
    let (tenant, payment_id, attempt_id, _s) = mint_session(&pool, RailId::RAZORPAY, true).await;
    vault_razorpay(&pool, tenant.as_str()).await;
    let fake = rails::razorpay::FakeRazorpay::default();
    fake.set_retrieve(200, &razorpay_fixture("psync_created.json"));
    let remote = RazorpayRemote::fake(pool.clone(), SecretBox::testing_fallback_key(), fake);
    psync::process_batch(&pool, &remote).await.unwrap();
    let ast: String = sqlx::query_scalar("SELECT status FROM pay_rs.attempts WHERE id = $1")
        .bind(attempt_id.as_uuid())
...
#[tokio::test]
async fn razorpay_remote_expired_stays_session_live() {
    let _g = PSYNC.lock().await;
    let pool = pool().await;
    let (tenant, payment_id, attempt_id, _s) = mint_session(&pool, RailId::RAZORPAY, true).await;
    vault_razorpay(&pool, tenant.as_str()).await;
    let fake = rails::razorpay::FakeRazorpay::default();
    fake.set_retrieve(200, &razorpay_fixture("psync_expired.json"));
    let remote = RazorpayRemote::fake(pool.clone(), SecretBox::testing_fallback_key(), fake);
    psync::process_batch(&pool, &remote).await.unwrap();
    let ast: String = sqlx::query_scalar("SELECT status FROM pay_rs.attempts WHERE id = $1")
        .bind(attempt_id.as_uuid())
