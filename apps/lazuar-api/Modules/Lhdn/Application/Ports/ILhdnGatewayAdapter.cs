...
public record LhdnCancelResult(bool Success, string? Status, string? ErrorMessage);
...
public interface ILhdnGatewayAdapter
{
    ...
    Task<LhdnCancelResult> CancelDocumentAsync(string clientId, string token, string uuid, string reason, bool isIntermediary, string? tenantTin, CancellationToken ct = default);
}
