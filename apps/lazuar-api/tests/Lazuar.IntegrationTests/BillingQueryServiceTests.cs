...
{
    // Uses the standard local dev database defined in your docker-compose / appsettings
    private readonly string _connectionString = "Host=localhost;Port=5432;Database=lazuar_mvp;Username=postgres;Password=postgres;";
    ...
    public async Task GetFinancialSummaryAsync_ShouldCalculateNetRevenueCorrectly_AndIgnoreOperationalExpenses()
    {
        ...
        {
            // Cleanup: Delete the test data to keep the local database pristine
            using var cmd = new NpgsqlCommand(@"DELETE FROM billing.""LedgerEntries"" WHERE ""OrganizationId"" = @OrgId;", connection);
            cmd.Parameters.AddWithValue("OrgId", orgId);
            await cmd.ExecuteNonQueryAsync();
        }
    }
