...
import { LogOut, PanelLeftClose, PanelLeftOpen, Server, ChevronDown, ChevronsUpDown } from "lucide-react";
          ...
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2, ease: "easeInOut" }} className="overflow-hidden">
            <div className="flex flex-col py-1 space-y-0.5">
              {links.map((link) => {
                ...
                return (
                  <Link key={link.href} to={link.href} onClick={() => isMobile && closeSidebar()} className={cn("flex h-8 w-full items-center pl-[48px] pr-4 text-[13px] transition-colors focus:outline-none", isExactActive ? "text-[#09090b] font-medium bg-[#f4f4f5]" : "text-[#71717a] hover:text-[#09090b] hover:bg-[#fafafa]")}>
                    {link.label}
                  </Link>
                );
              })}
...
export default function Sidebar({ isOpen, setIsOpen, isMobile, user, onLogout }: SidebarProps) {
  ...
  const [openSections, setOpenSections] = useState<Record<string, boolean>>(() => {
    try {
      const stored = localStorage.getItem("lazuar-admin-sidebar-sections");
  ...
  useEffect(() => {
    localStorage.setItem("lazuar-admin-sidebar-sections", JSON.stringify(openSections));
  }, [openSections]);

  useEffect(() => {
    MODULES.forEach(mod => {
      if (location.pathname.startsWith(mod.basePath) && !openSections[mod.id]) {
