...
export default function SubscribersPage() {
  ...
  const actionMutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      if (variables.action === "record-payment") setIsPaymentModalOpen(false);
      // Fetch latest state for the slide over to update
      setSelectedSub(prev => prev ? { ...prev, status: variables.action === 'cancel' ? 'CANCELLED' : prev.status } : null);
    },
  ...
  return (
    <PageLayout 
      ...
      actionButton={
        <button onClick={handleExport} className="h-9 px-4 bg-white border border-[#e5e5e5] text-[#09090b] text-[11px] font-bold uppercase tracking-widest flex items-center gap-2 hover:bg-[#f4f4f5] transition-colors">
    ...
    >
              ...
              {isLoading ? (
                ...
                displayedSubscribers.map((sub) => (
                  <tr key={sub.id} onClick={() => setSelectedSub(sub)} className="hover:bg-[#fafafa] transition-colors cursor-pointer group">
                    ...
                    <td className="px-5 py-3.5 text-right font-mono text-[#52525b] text-[11px]">
      ...
      {selectedSub && (
        <div className="fixed inset-0 z-50 flex justify-end">
          <div className="absolute inset-0 bg-black/20 backdrop-blur-sm transition-opacity" onClick={() => setSelectedSub(null)} />
          <div className="relative w-full max-w-md bg-white border-l border-[#e5e5e5] h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
            <div className="flex items-center justify-between p-5 border-b border-[#e5e5e5] bg-[#fafafa]">
              <div>
              ...
              <button onClick={() => setSelectedSub(null)} className="p-1.5 text-[#a1a1aa] hover:bg-[#e5e5e5] hover:text-[#09090b] transition-colors rounded-sm"><X size={16} /></button>
            </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button 
                    ...
                    className="h-8 border border-[#e5e5e5] bg-white text-[10px] font-bold uppercase tracking-widest text-[#09090b] hover:bg-[#f4f4f5] transition-colors"
                  >
                    Log Payment
                  </button>
                  <button 
                  ...
                  <button 
                    ...
                    className="h-8 border border-amber-200 bg-amber-50 text-[10px] font-bold uppercase tracking-widest text-amber-700 hover:bg-amber-100 transition-colors"
                  >
                    Cancel Sub
                  </button>
                  <button 
                    onClick={() => { if (window.confirm("CRITICAL: Ban user and revoke all access immediately?")) actionMutation.mutate({ action: "ban" }); }}
                    className="h-8 border border-rose-200 bg-rose-50 text-[10px] font-bold uppercase tracking-widest text-rose-700 hover:bg-rose-100 transition-colors flex items-center justify-center gap-1"
                  >
                    <AlertTriangle size={12} /> Ban User
                  </button>
                </div>
              </div>
                      ...
                      <div key={payment.id} className="p-3 border border-[#e5e5e5] bg-[#fafafa] flex items-center justify-between">
                        ...
                        {payment.status === "CONFIRMED" && payment.amount > 0 && (
                          <button 
                            ...
                            className="text-[10px] font-bold uppercase tracking-widest text-rose-600 hover:underline"
                          >
                            Refund
                          </button>
                        )}
      ...
      {isPaymentModalOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/20 backdrop-blur-sm" onClick={() => setIsPaymentModalOpen(false)} />
          <form 
          ...
          >
            ...
            <div className="p-5 space-y-4">
              <div className="space-y-1.5">
                ...
                <input required type="number" step="0.01" value={paymentAmount} onChange={e => setPaymentAmount(e.target.value)} className="w-full h-9 border border-[#e5e5e5] px-3 text-[13px] focus:outline-none focus:border-[#09090b]" />
              </div>
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold uppercase tracking-widest text-[#71717a]">Method *</label>
                <select value={paymentMethod} onChange={e => setPaymentMethod(e.target.value)} className="w-full h-9 border border-[#e5e5e5] px-3 text-[13px] focus:outline-none focus:border-[#09090b]">
              ...
              <div className="space-y-1.5">
                ...
                <input value={paymentRef} onChange={e => setPaymentRef(e.target.value)} className="w-full h-9 border border-[#e5e5e5] px-3 font-mono text-[13px] focus:outline-none focus:border-[#09090b]" />
              </div>
            </div>
            <div className="p-4 border-t border-[#f4f4f5] bg-[#fafafa]/50 flex justify-end gap-2">
              <button type="button" onClick={() => setIsPaymentModalOpen(false)} className="px-4 h-8 text-[11px] font-bold uppercase tracking-widest text-[#71717a] hover:text-[#09090b]">Cancel</button>
              <button type="submit" disabled={actionMutation.isPending} className="px-5 h-8 bg-[#09090b] text-white text-[11px] font-bold uppercase tracking-widest hover:bg-[#27272a] disabled:opacity-50">
                {actionMutation.isPending ? <Loader2 size={13} className="animate-spin" /> : "Save Payment"}
              </button>
            </div>
          </form>
        </div>
