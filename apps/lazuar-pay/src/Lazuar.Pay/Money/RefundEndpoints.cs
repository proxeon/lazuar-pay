...
internal static class RefundEndpoints
{
        ...
        CancellationToken ct)
    {
        ...
        var already = await db.Refunds.Where(x => x.ChargeId == charge.Id && x.Status == "succeeded")
            .SumAsync(x => x.Amount, ct);
        var remaining = charge.Amount - already;
        if (remaining <= 0)
        {
            return PayErrors.Status(409, "Conflict", "already refunded");
        }

        var amount = body?.Amount ?? remaining;
        ...
        var seq = await db.DocumentSequences.FindAsync([orgId, "REF", year], ct);
        if (seq is null)
        {
            seq = new DocumentSequenceRow { OrgId = orgId, Series = "REF", YearMyt = year, LastN = 0 };
            db.DocumentSequences.Add(seq);
        }

        seq.LastN += 1;
        var number = $"REF-{year}-{seq.LastN:00000}";
