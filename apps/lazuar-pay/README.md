...
Scale (plans/031/06): the public `start`/`confirm` rate limit (`Pay:StartMaxPerMinute`, default 20 per token) and the per-checkout/per-link mutexes are **per process** — N replicas behind a load balancer each hold their own budget, so size the limit by replica count or run one replica. Correctness across replicas never depends on them: compare-and-set status transitions, unique indexes (`charges.CheckoutId`, idempotency keys, one late-pay refund per checkout), and `FOR UPDATE` capacity checks live in Postgres.

Drift telemetry (plans/031/04): every inbound webhook records one `psp_parse_outcome{provider,outcome}` measurement — `ok | ignored | verify_failed | dedupe | checkout_missing | amount_mismatch | currency_mismatch | secret_unavailable` — and ignored events persist their `ignore_reason` on the event row. Alert shape: `verify_failed`/`checkout_missing`/`amount_mismatch` rate above zero per provider, or the ignored ratio approaching 1 for an hour; `secret_unavailable` is the WrapKey-rotation signature.
