...
function PortalContent({ tenantSlug }: { tenantSlug: string }) {
  ...
  const handleUpdateContact = async (e: React.FormEvent) => {
    e.preventDefault();
    toast.success("Note: Global identity updates must be done via the Identity Hub.");
  };
    ...
    return (
      <div className="min-h-screen flex flex-col bg-zinc-50 dark:bg-black">
        ...
        <main className="flex-1 w-full max-w-3xl mx-auto px-4 py-16 text-center">
          ...
          <p className="text-muted-foreground mb-8 text-sm">Please manage your master identity and active subscriptions from your global profile.</p>
          <a href={`${AUTH_URL}/profile`} className="h-12 px-8 inline-flex items-center bg-foreground text-background text-sm font-bold uppercase tracking-widest rounded-none hover:bg-foreground/90 transition-colors">
            Go to Identity Hub
          </a>
        </main>
      </div>
      ...
      <main className="flex-1 w-full max-w-3xl mx-auto px-4 py-8 md:py-12 space-y-12">
        <div className="bg-card border border-border/60 shadow-sm p-6 rounded-none">
          ...
          <form onSubmit={handleUpdateContact} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            ...
            <a href={`${AUTH_URL}/profile`} className="flex items-center justify-center h-10 rounded-none border border-border/60 bg-background text-[10px] font-bold uppercase tracking-widest text-foreground hover:bg-secondary transition-colors">
              Manage Details
            </a>
          </form>
        </div>
