...
- [x] `RequireMemberAsync`: if Bearer is `lzr_sk_`, **do not** call `CheckMemberAsync`
- [x] Instead call One `GET /me` with the same Authorization (reuse whoami client)
- [x] One 401 → Pay 401
- [x] One timeout / 5xx → Pay 503 (existing transport mapping)
- [x] JWT path **unchanged**: still `authz/check` omit `user_id`, then writer overlay on `/me` for writers

## M12.2 Must not

- [x] Do not send key id as `authz/check` `user_id` (One 400 impersonation)
- [x] Do not require the second app to pass Ada’s Zitadel sub
- [x] Do not skip One entirely for keys

## M12.3 Tests (expect still not writer until M14)

- [x] Key `/v1/orgs/t1/ready` after M13 will be 200 — until M13, may 403 if bound-tenant not wired
- [x] JWT `"Bearer tok"` still POSTs `authz/check` (assert Fake One last path)
