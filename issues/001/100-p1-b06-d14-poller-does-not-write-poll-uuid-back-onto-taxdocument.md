...
head: "297ba98"
resolved_branch: fix/100-poll-uuid-on-valid
---
...
- **Resolved on:** `fix/100-poll-uuid-on-valid`

`MarkAsValid` writes the poll UUID so GET `/lhdn/documents/{id}` can build a QR when submit missed `acceptedDocuments`.
